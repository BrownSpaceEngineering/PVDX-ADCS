...
function simulate_1vec()
    ...
    true_omega = deg2rad([1, 0.05, -1]);%true angular velocity in the reference frame
    ...
    R = eye(6) * 5;%measurement noise matrix for 1 vector
    ...
    for i = 1:1000 %iterate
        ...
        last_body = body;
        last_reference = reference;
        reference = reference + normrnd(0, 0.05, [1, 3]);
        if mod(i, 50) == 0
            disp("Iteration");
            %disp(current_guess)
            %disp(true_rotation)
        ...
        total_err = total_err + rad2deg(Quaternion.quat_diff(true_body_to_ref, current_guess));
