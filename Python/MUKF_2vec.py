...
def get_measurements(quat_sigmas):
    ...
    for sigma in quat_sigmas:
        ...
        expected_reading_two = rotate(np.array([1, 1, 1]), ECI_to_body)
...
if __name__ == '__main__':
    ...
    rot = true_rot.elements + np.random.normal(loc = 0, scale = 0.5, size = 4)
    rot = Quaternion(rot).normalised
    P = np.eye(6) * 0.1
    state = np.zeros(6)

    true_angular_velocity = np.array([0.1, -0.1, 0.1])#30 degrees per second, which is too fast for us but is good for simulation purposes
    ...
    for i in range(1000):
        ...
        given_reading_one = rotate(np.array([1,0, 0]) + np.random.normal(loc = 0, scale = 0.001, size = 3), ref_to_body)
        given_reading_two = rotate(np.array([1, 1,1]) + np.random.normal(loc = 0, scale = 0.001, size = 3), ref_to_body)
